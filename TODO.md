...
- [x] **动态页面安全边界与数据分支。** 修复说说页和友情链接页的数据来源判断，统一对动态内容做 HTML 转义、协议校验和外链安全处理；搜索结果与最新评论也不再直接信任外部 HTML（2026-09-12）。

- [x] **结构化数据开关与首屏可用性。** `structured_data.enable` 现在真正控制 JSON-LD 输出；新增 skip link、键盘焦点样式、reduced-motion 兜底，并优先加载首屏关键封面图片（2026-09-12）。

- [x] **基础验证与发布流程。** 增加 smoke test、Hexo fixture CI、安全策略文档和无障碍/性能指南；修复 CDN 路径清理正则（2026-09-12）。
